    ...
    <script>
        ...
        function initSelect2(){
            ...
            $("#select2-permissions").on("select2:select", (e) => {
                ...
                @this.call('selectPermission', data.text);
                
            })
            $("#select2-permissions").on("select2:unselect", (e) => {
                let data = e.params.data;
                @this.call('unselectPermission', data.text);
        ...
        Livewire.on('setSelectedPermissions', (data) => {
            ...
            data.forEach(permissions => {
                permissions.forEach(permission => {
                    $('#select2-permissions').append(`<option value="${permission.name}" selected>${permission.name}</option>`);
                })
            });
        });

        Livewire.on('reSelect2', (data) => {
            initSelect2();
        });
    </script>
@endpush
