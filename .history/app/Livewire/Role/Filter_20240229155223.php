...
class Filter extends Component
{
    ...
    public function selectPermission($permission){
        $this->dispatch('onFailSweetAlert', $permission);
    }
    public function unselectPermission($permission){
        $existingArray = ['apple', 'banana', 'orange'];

        // Data to be appended
        $newData = ['banana', 'grape', 'kiwi', 'orange'];

        // Merge the arrays and remove duplicates
        $mergedArray = array_unique(array_merge($existingArray, $newData));
        $this->dispatch('consoleLog', $mergedArray);
        $this->dispatch('onFailSweetAlert', $permission);
    }
